...
include 'classes/TeamAbbrHolder.php';
...
if (file_exists($Fnm)) {
	...
	while(list($cle,$val) = myEach($tableau)) {
		...
		if(substr_count($val, '<TD>Pro Payroll</TD>') && $b && $d) {
			...
			$propayroll = format_money_no_dec($propayroll,true);
		}
		if(substr_count($val, '<TD>Farm Payroll</TD>') && $b && $d) {
			$pos = strpos($val, '</TD></TR>');
			$pos = $pos - 30;
			$farmpayroll = substr($val, 30, $pos);
			$farmpayroll = format_money_no_dec($farmpayroll,true);
			break 1;
		}
	}
}
...
$Fnm = getLeagueFile('PlayerVitals');
$a = 0;
$b = 0;
$d = 1;
$njoueurs = 0;
...
if (file_exists($Fnm)) {
	...
	while(list($cle,$val) = myEach($tableau)) {
		...
		if($a == 3 && $b && $d) {
			...
			$vitalsSalaire = format_money_no_dec($vitalsSalaire,true);
			$a++;
		}
...
$times = 0;
while($d == 1) {
    if($times == 0) $Fnm = getLeagueFile('TeamScoring');//$Fnm = $folder.$folderLeagueURL.'TeamScoring.html';
    if($times == 1) $Fnm = _getLeagueFile('TeamScoring');//$Fnm = $folder.$folderLeagueURL2.'TeamScoring.html';
	$a = 0;
	$b = 0;
	$e = 0;
	$f = 0;
...
if(isset($propayroll)) {
	...
	$restant = format_money_no_dec($restant);
	...
	if(isset($statsFwdName)) {
		...
		echo '<a href="CareerLeaders.php?team='.htmlspecialchars($currentTeam).'"><div style="clear:both;">';
			...
			echo '<tr>
				...
				<th data-toggle="tooltip" data-placement="top" title="'.$scoringAssits.'">A</th>
				<th data-toggle="tooltip" data-placement="top" title="Points">P</th>
				<th data-toggle="tooltip" data-placement="top" title="'.$scoringDiff.'">+/-</a></th>
